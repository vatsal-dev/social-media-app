import { useState } from "react";
import {
  Box,
  IconButton,
  InputBase,
  Typography,
  Select,
  MenuItem,
  FormControl,
  useTheme,
  useMediaQuery,
} from "@mui/material";
import { DarkMode, LightMode, Menu, Close, GitHub } from "@mui/icons-material";
import { useDispatch, useSelector } from "react-redux";
import { setMode, setLogout } from "state";
import { useNavigate } from "react-router-dom";
import FlexBetween from "components/FlexBetween";
import { Tooltip } from "@mui/material";
import "../../LoginPage.css";

const Navbar = () => {
  // State and hooks initialization
  const [isMobileMenuToggled, setIsMobileMenuToggled] = useState(false);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const user = useSelector((state) => state.user);
  const isNonMobileScreens = useMediaQuery("(min-width: 1000px)");

  // Extracting theme colors from Material-UI theme
  const theme = useTheme();
  const neutralLight = theme.palette.neutral.light;
  const dark = theme.palette.neutral.dark;
  const background = theme.palette.background.default;

  const alt = theme.palette.background.alt;

  // Generate the user's full name for the select dropdown
  const fullName = `${user.firstName} ${user.lastName}`;

  return (
    <nav style={{ zIndex: isNonMobileScreens ? "0" : "101" }}>
      <FlexBetween padding="0.5rem 6%" backgroundColor={alt}>
        <FlexBetween gap="1.50rem">
          {/* Brand logo and search bar */}
          <Typography
            fontWeight="bold"
            fontSize="clamp(1rem, 2.75rem, 3rem)"
            onClick={() => navigate("/home")}
            sx={{
              background: `linear-gradient(45deg, #f09433, #e6683c, #dc2743, #cc2366, #bc1888, #8a0a86, #5a0a92, #2c0692, #0c2461)`,
              backgroundSize: "300% 300%",
              //backgroundRepeat: "repeat",
              backgroundClip: "text",
              WebkitBackgroundClip: "text",
              WebkitTextFillColor: "transparent",

              "&:hover": {
                color: "white",
                cursor: "pointer",
              },
            }}
          >
            BuZZ
          </Typography>
          {/* {isNonMobileScreens && (
          <FlexBetween
            backgroundColor={neutralLight}
            borderRadius="9px"
            gap="3rem"
            padding="0.1rem 1.5rem"
          >
            <InputBase placeholder="Search..." />
            <IconButton>
              <Search />
            </IconButton>
          </FlexBetween>
        )} */}
        </FlexBetween>

        {/* DESKTOP NAV */}
        {isNonMobileScreens ? (
          // Desktop navigation with icons, GitHub link, and select dropdown
          <FlexBetween gap="2rem">
            <IconButton onClick={() => dispatch(setMode())}>
              {theme.palette.mode === "dark" ? (
                <DarkMode sx={{ fontSize: "25px" }} />
              ) : (
                <LightMode sx={{ color: dark, fontSize: "25px" }} />
              )}
            </IconButton>

            {/* GitHub link */}
            <Tooltip title="Check out my GitHub profile">
              <a
                href="https://github.com/vatsal-dev"
                target="_blank"
                rel="noopener noreferrer"
              >
                <IconButton>
                  <GitHub sx={{ fontSize: "25px", color: "red" }} />
                </IconButton>
              </a>
            </Tooltip>

            {/* User select dropdown */}
            <FormControl variant="standard" value={fullName}>
              <Select
                value={fullName}
                sx={{
                  backgroundColor: neutralLight,
                  width: "150px",
                  borderRadius: "0.75rem",
                  p: "0.60rem 1rem",
                  "& .MuiSvgIcon-root": {
                    pr: "0.25rem",
                    width: "3rem",
                  },
                  "& .MuiSelect-select:focus": {
                    backgroundColor: neutralLight,
                  },
                }}
                input={<InputBase />}
              >
                <MenuItem value={fullName}>
                  <Typography>{fullName}</Typography>
                </MenuItem>
                <MenuItem onClick={() => dispatch(setLogout())}>
                  Log Out
                </MenuItem>
              </Select>
            </FormControl>
          </FlexBetween>
        ) : (
          // Mobile navigation with the menu icon (for non-desktop screens)
          <IconButton
            onClick={() => setIsMobileMenuToggled(!isMobileMenuToggled)}
          >
            <Menu />
          </IconButton>
        )}

        {/* MOBILE NAV */}
        {!isNonMobileScreens && isMobileMenuToggled && (
          // Mobile navigation menu with close icon, icons, GitHub link, and select dropdown
          <Box
            position="fixed"
            right="0"
            bottom="0"
            height="100%"
            zIndex="101"
            maxWidth="500px"
            minWidth="300px"
            backgroundColor={background}
          >
            {/* CLOSE ICON */}
            <Box display="flex" justifyContent="flex-end" p="1rem">
              <IconButton
                onClick={() => setIsMobileMenuToggled(!isMobileMenuToggled)}
              >
                <Close />
              </IconButton>
            </Box>

            {/* MENU ITEMS */}
            <FlexBetween
              display="flex"
              flexDirection="column"
              justifyContent="center"
              alignItems="center"
              gap="3rem"
            >
              <IconButton
                onClick={() => dispatch(setMode())}
                sx={{ fontSize: "25px" }}
              >
                {theme.palette.mode === "dark" ? (
                  <DarkMode sx={{ fontSize: "25px" }} />
                ) : (
                  <LightMode sx={{ color: dark, fontSize: "25px" }} />
                )}
              </IconButton>

              {/* GitHub link */}
              <a href="https://github.com/vatsal-dev">
                <IconButton>
                  <GitHub sx={{ fontSize: "25px" }} />
                </IconButton>
              </a>

              {/* User select dropdown */}
              <FormControl variant="standard" value={fullName}>
                <Select
                  value={fullName}
                  sx={{
                    backgroundColor: neutralLight,
                    width: "150px",
                    borderRadius: "0.25rem",
                    p: "0.25rem 1rem",
                    "& .MuiSvgIcon-root": {
                      pr: "0.25rem",
                      width: "3rem",
                    },
                    "& .MuiSelect-select:focus": {
                      backgroundColor: neutralLight,
                    },
                  }}
                  input={<InputBase />}
                >
                  <MenuItem value={fullName}>
                    <Typography>{fullName}</Typography>
                  </MenuItem>
                  <MenuItem onClick={() => dispatch(setLogout())}>
                    Log Out
                  </MenuItem>
                </Select>
              </FormControl>
            </FlexBetween>
          </Box>
        )}
      </FlexBetween>
    </nav>
  );
};

// Export the Navbar component as the default export
export default Navbar;
